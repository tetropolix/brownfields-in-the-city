...
class Brownfield(Base):
    ...
    color_id = Column(Integer, ForeignKey("brownfields.colors.id"), nullable=False)
    ownership_type_id = Column(
        Integer, ForeignKey("brownfields.ownership_types.id"), nullable=False
    )
    original_functional_utilization_id = Column(
        Integer,
        ForeignKey("brownfields.original_functional_utilizations.id"),
        nullable=False,
    )
    utilization_id = Column(
        Integer, ForeignKey("brownfields.utilizations.id"), nullable=False
    )
    area_size_id = Column(
        Integer, ForeignKey("brownfields.area_sizes.id"), nullable=False
    )
    location_id = Column(
        Integer, ForeignKey("brownfields.locations.id"), nullable=False
    )
    degradation_level_id = Column(
        Integer, ForeignKey("brownfields.degradation_levels.id"), nullable=False
    )
    residentional_area_category_id = Column(
        Integer,
        ForeignKey("brownfields.residentional_area_categories.id"),
        nullable=False,
    )
    settlement_id = Column(
        Integer, ForeignKey("brownfields.settlements.id"), nullable=False
    )
    infrastructure_availability_id = Column(
        Integer,
        ForeignKey("brownfields.infrastructure_availabilities.id"),
        nullable=False,
    )
    natural_and_architectural_value_id = Column(
        Integer,
        ForeignKey("brownfields.natural_and_architectural_values.id"),
        nullable=False,
    )
    revitalization_id = Column(
        Integer, ForeignKey("brownfields.revitalizations.id"), nullable=False
    )
    economic_potential_id = Column(
        Integer, ForeignKey("brownfields.economic_potentials.id")
    )
    environmental_burden_inclusion_id = Column(
        Integer, ForeignKey("brownfields.environmental_burden_inclusions.id")
    )

    color = relationship("Color", back_populates="brownfields", lazy="joined")
    ownership_type = relationship(
        "OwnershipType", back_populates="brownfields", lazy="joined"
    )
    original_functional_utilization = relationship(
        "OriginalFunctionalUtilization", back_populates="brownfields", lazy="joined"
    )
    utilization = relationship(
        "Utilization", back_populates="brownfields", lazy="joined"
    )
    ...
    degradation_level = relationship(
        "DegradationLevel", back_populates="brownfields", lazy="joined"
    )
    residentional_area_category = relationship(
        "ResidentionalAreaCategory", back_populates="brownfields", lazy="joined"
    )
    settlement = relationship("Settlement", back_populates="brownfields", lazy="joined")
    infrastructure_availability = relationship(
        "InfrastructureAvailability", back_populates="brownfields", lazy="joined"
    )
    natural_and_architectural_value = relationship(
        "NaturalAndArchitecturalValue", back_populates="brownfields", lazy="joined"
    )
    revitalization = relationship(
        "Revitalization", back_populates="brownfields", lazy="joined"
    )
    economic_potential = relationship(
        "EconomicPotential", back_populates="brownfields", lazy="joined"
    )
    environmental_burden_inclusion = relationship(
        "EnvironmentalBurdenInclusion", back_populates="brownfields", lazy="joined"
    )
...
class Color(Base):
    ...
    hex_value = Column(String(8), nullable=False)

    brownfields = relationship("Brownfield", back_populates="color")

    def __repr__(self):
        return f"Brownfield color (id = {self.id}) with name = {self.name} and hex value = {self.hex_value}"
...
class OriginalFunctionalUtilization(Base):
    ...
    brownfields = relationship(
        "Brownfield", back_populates="original_functional_utilization"
    )
...
class ResidentionalAreaCategory(Base):
    ...
    brownfields = relationship(
        "Brownfield", back_populates="residentional_area_category"
    )
...
class InfrastructureAvailability(Base):
    ...
    brownfields = relationship(
        "Brownfield", back_populates="infrastructure_availability"
    )
...
class NaturalAndArchitecturalValue(Base):
    ...
    brownfields = relationship(
        "Brownfield", back_populates="natural_and_architectural_value"
    )
...
class EconomicPotential(Base):
    ...
    )  ## TO DO - value is nullable because eventual values = TBA
...
class EnvironmentalBurdenInclusion(Base):
    ...
    brownfields = relationship(
        "Brownfield", back_populates="environmental_burden_inclusion"
    )
